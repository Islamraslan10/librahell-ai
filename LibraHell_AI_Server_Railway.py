...
def build_features(data):
    ...
    return np.array([signal, quality/100.0, angle_strength,
                     sl_dist, tp_dist, rr_ratio, lot,
                     open_trades/5.0, daily_loss/10.0, balance/10000.0], dtype=np.float32)

def rule_based_predict(data, features):
    confidence = 0.50
    reason = []
    ...
    if open_trades <= 1:  confidence += 0.05; reason.append("Few trades")
    if quality < 65:      confidence -= 0.20; reason.append("Low quality")
    if rr_ratio < 1.5:    confidence -= 0.15; reason.append("Poor RR")
    if daily_loss >= 2.5: confidence -= 0.25; reason.append("Near daily limit")
    confidence = max(0.0, min(1.0, confidence))
    return {"approved": confidence >= MIN_CONFIDENCE,
...
def health():
    return jsonify({"status": "running", "version": "LibraHell AI v1.0",
...
if __name__ == "__main__":
    ...
    log.info(f"Starting LibraHell AI on port {port}")
